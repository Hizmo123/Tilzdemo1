...
function InviteForm() {
  ...
  return (
    <div className="rounded-[var(--radius-card)] border border-line bg-surface p-6">
      ...
      {lastUrl && (
        <div className="mt-4 rounded-[var(--radius-sm)] bg-pine-soft p-3">
          <p className="text-sm text-pine-deep font-medium mb-1">
            Invite created — share this link:
          </p>
          <div className="flex items-center gap-2">
            <code className="flex-1 text-xs break-all bg-surface rounded-[var(--radius-xs)] px-2 py-1.5">
              {lastUrl}
            </code>
            <button
              onClick={() => navigator.clipboard?.writeText(lastUrl)}
              className="text-xs rounded-[var(--radius-xs)] border border-line bg-surface px-2.5 py-1.5 hover:border-ink/30"
            >
              Copy
            </button>
          </div>
