...
// A stand that isn't bound to an active table (not yet assigned, or its
// table's own QR token got revoked) shows a plain "not set up" message —
// NEVER anything that looks like it could take an order.
export default async function StandPage({
  ...
  const result = await resolveStand(id);
